...
to recalculate-variables
  ...
  ; then multiply by grid cells / km to get budget
  ; in grid cell length
...
;  cells / village * ha / cell * T / ha *  people / T
  ...
  let people-per-village-connected initial-people-per-village + ( max-hectares-per-village * yield-connected * (1 / crops-per-person) )
  let people-per-village-disconnected initial-people-per-village + ( max-hectares-per-village * yield-disconnected * (1 / crops-per-person) )
...
to normalize-criteria-values
  if ( count patches with [ roadsPaved = 0 ] > 0 ) [
    ...
    ask roads [
      ...
      ifelse ( my-length > 0 ) [
        ; normalizes avoided flood probability because roads are different lengths
...
to pave-roads
  ...
  ifelse ( count roads with [ my-paved = 0 and ( my-max-villages > 0 or my-max-storage > 0) and roadsConnected? = true ] > 0 ) [
    ...
    while [ current-roads-budget > 0 and count roads with [ my-paved = 0 and ( my-max-villages > 0 or my-max-storage > 0) and roadsConnected? = true ] > 0 ] [
      ...
      ifelse [ my-length ] of road-to-pave < current-roads-budget [
